...
head:
  ...
  - - meta
    - name: description
      content: 基于《计算机网络》教材的学习笔记，梳理术语与分层模型等核心知识点，便于期末复习与面试巩固。
---
...
# 数据链路层与物理层核心知识点笔记（含以太网）

# 一、物理层核心知识点

## 1.1 核心目标

- 透明地传送比特流：上层（数据链路层）只需将0/1比特流交给物理层，无需关心传输细节，接收端物理层会原样将比特流回传给上层（“透明”即对上层屏蔽传输过程）。

- 屏蔽底层差异：屏蔽网线、光纤、无线等传输媒体，以及不同硬件接口的差异，让数据链路层无需适配不同传输介质。

## 1.2 四大接口特性（物理层规范核心）

|特性类型|核心作用|典型例子（以太网RJ45接口）|
|---|---|---|
|机械特性|规定硬件接口的物理形状、尺寸、触点数量|RJ45接口为长方形，8个金属触点，网线插头可精准插入|
|电气特性|规定接口电缆的电压范围、信号类型（0/1对应的信号形式）|以太网10BASE-T标准：差分电信号，电压范围±2V，正电压差代表1，负电压差代表0|
|功能特性|规定接口每条线的功能含义|RJ45网线8根线中，2对用于传输（1对TX发送线、1对RX接收线），其余为备用或地线|
|过程特性|规定不同功能事件的执行顺序、时序要求|以太网采用曼彻斯特编码，每个比特中间有电平跳变（同步时钟+表示数据）；10BASE-T比特率10Mbps，每100ns传输1个比特|
## 1.3 比特流→调制电信号的完整过程（以太网10BASE-T为例）

核心逻辑：物理层硬件（网卡PHY芯片）将上层传来的数字逻辑比特流，转换成适合网线传输的差分电信号，全程遵循物理层电气和时序规范。

1. 前提：数据链路层将以太网帧（含MAC地址、CRC校验码等）转换为二进制比特流（如01001101...），传递给物理层（此时为网卡内部的逻辑电平，3.3V代表1，0V代表0，无法直接远距离传输）。

2. 步骤1：编码（解决时钟同步问题）
       

    - 采用曼彻斯特编码：每个比特的中间时刻必须有一次电平跳变，跳变方向表示比特值（低→高跳变为0，高→低跳变为1）。

    - 作用：既携带数据（跳变方向），又携带时钟同步信号（跳变本身），确保接收端能准确识别每个比特的起始和结束位置。

    - 示例：原始比特流1 0 1 → 编码后电平变化：高→低、低→高、高→低。

3. 步骤2：信号转换（逻辑电平→差分电信号）
        

    - 逻辑电平转单端电信号：PHY芯片将编码后的高低逻辑电平，转换为以太网规范的电压范围（±2V）。

    - 单端转差分电信号：为提升抗干扰能力，将单端信号转换为差分信号（通过TX+和TX-两根线的电压差表示信号）。规则：编码后的高电平→TX+与TX-产生+2V电压差（TX+=+1V，TX-=-1V）；低电平→产生-2V电压差（TX+=-1V，TX+=+1V）。

4. 步骤3：传输：按照功能特性和时序规范，通过网卡的TX发送线将差分电信号输出到RJ45接口，再通过网线传输到交换机/接收端。

接收端反向过程：接收端物理层通过RX接收线接收差分电信号→检测电压差还原高低电平→通过曼彻斯特编码还原原始比特流→传递给数据链路层。

# 二、以太网核心知识点

## 2.1 以太网定义

以太网（Ethernet）是全球最主流的局域网（LAN）技术标准，定义了局域网数据传输的完整技术规则，包括数据链路层协议和物理层接口规范。日常网线连接电脑、交换机、路由器的组网方式，就是以太网的典型应用。

标准归属：由IEEE 802.3协议族定义，常见版本有10BASE-T（10M双绞线以太网）、100BASE-TX（百兆以太网）、1000BASE-T（千兆以太网）。

## 2.2 以太网的分层归属

以太网标准跨数据链路层和物理层，两层职责明确：

|层级|以太网标准核心内容|核心职责|
|---|---|---|
|数据链路层|以太网帧格式（帧头：源/目的MAC地址、帧类型；帧尾：CRC校验码）、MAC寻址规则、冲突检测机制（早期共享式以太网）|解决“局域网内找谁传”和“传输内容是否损坏”|
|物理层|电气特性（电压范围、信号类型）、时序规范（编码方式、比特率）、机械特性（RJ45接口）、功能特性（TX/RX线定义）|解决“怎么把比特流转换成信号传输”|
关键结论：以太网的电气和时序规范属于物理层。

# 三、数据链路层与物理层协同传输（局域网示例）

场景：电脑A通过交换机向电脑B传输文本文件片段（聚焦两层协同，忽略上层协议）。

## 3.1 发送端（电脑A）

- 数据链路层：接收上层IP数据包→封装以太网帧（加帧头：源/目的MAC地址、帧类型；加帧尾：CRC校验码）→检查帧长度（46~1500字节，不足则填充）→将帧转换为比特流传递给物理层。

- 物理层：对比特流进行曼彻斯特编码→转换为差分电信号→通过TX线发送到交换机。

## 3.2 中间设备（交换机）

- 物理层：接收电信号→还原为比特流→传递给数据链路层。

- 数据链路层：解析帧头目的MAC地址→查询MAC地址表找到对应端口→将比特流传递给该端口的物理层。

- 物理层：再次将比特流转为差分电信号→发送到电脑B。

## 3.3 接收端（电脑B）

- 物理层：通过RX线接收电信号→还原为比特流→传递给数据链路层。

- 数据链路层：CRC校验（验证帧完整性）→解封装（去掉帧头帧尾）→提取IP数据包并传递给上层。

# 四、核心区别与关联总结

|对比维度|数据链路层|物理层|
|---|---|---|
|核心处理对象|帧（含MAC地址、CRC等）|比特流+物理信号（电/光/电磁波）|
|核心职责|封装/解封装帧、MAC寻址、CRC差错检测|屏蔽传输介质差异、比特流与物理信号转换、时钟同步|
|依赖标准|以太网数据链路层协议（IEEE 802.3）|以太网物理层规范（如10BASE-T的电气/时序要求）|
关联：数据链路层负责“找对目标+验证数据完整性”，物理层负责“将数据以信号形式传输”，两层协同完成局域网内的可靠数据传输，物理层为数据链路层提供透明的比特流传输服务。
